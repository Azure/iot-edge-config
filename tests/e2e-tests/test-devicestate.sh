...
function cleanup() {
	...
	then
		...
		curl -X DELETE -H "Authorization:Bearer $armToken" https://e2etest-iotc-iiot-asset-app.azureiotcentral.com/api/preview/devices/${device_id} 2>/dev/null
	else
	...
	then
		...
		curl -X DELETE -H "Authorization:Bearer $armToken" https://e2etest-iotc-iiot-asset-app.azureiotcentral.com/api/preview/apiTokens/${token_id} 2>/dev/null
	else
