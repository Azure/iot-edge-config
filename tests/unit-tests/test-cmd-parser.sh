...
test_all_legal_flags_cn() {
    # parse sample input to parser
    declare -A parsed_cmds="$(cmd_parser $@)"

    # compare output, should be 'x;y;z', true
    if [[ "${parsed_cmds['CONNECTION_STRING']}" != "x;y;z" || "${parsed_cmds['VERBOSE_LOGGING']}" != true ]];
    then
        echo "Failed to pass unit test 'test_all_legal_flags_cn' in test-cmd-parser.sh. Non-empty result: ${parsed_cmds[*]}"
    fi
}

test_all_legal_flags_dps() {
    # parse sample input to parser
    declare -A parsed_cmds="$(cmd_parser $@)"
    ...
    then
        echo "Failed to pass unit test 'test_all_legal_flags_dps' in test-cmd-parser.sh. Non-empty result: ${parsed_cmds[*]}"
    fi
}
...
# run all tests
test_illegal_flags
test_all_legal_flags_cn -c 'x;y;z' -v
test_all_legal_flags_dps -dp 1 -ap 5 -s 2 -r 4 -k 3 -v 6
test_extra_arguments
